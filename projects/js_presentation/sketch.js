...
const SHATTER_VELOCITY_THRESHOLD = 30;  
...
window.addEventListener('load', () => {
    ...
    header.addEventListener('mousedown', (e) => {
        ...
        // Switch from right/top to left/top positioning for dragging
    ...
    document.addEventListener('mousemove', (e) => {
        if (isPanelDragging) {
            ...
            // Simple boundary check to keep some part of panel on screen
            const maxX = window.innerWidth - 50;
            const maxY = window.innerHeight - 50;
            
            panel.style.left = Math.min(Math.max(x, -400), maxX) + 'px';
            panel.style.top = Math.min(Math.max(y, 0), maxY) + 'px';
        }
    });
    ...
    3: {  
        ...
        formulas: [  
            {  
                ...
                latex: String.raw`\vec{g} = \begin{pmatrix} g_x \\ g_y \end{pmatrix}, \quad g_{x,y} \in \{0, \pm 0.3\}`  
            },  
            {  
...
// ============================================================  
// Tech Panel Update Function (Fixed)  
// ============================================================  
...
function updateTechPanel() {  
    ...
    if (details.formulas && details.formulas.length > 0) {  
        html += '<div class="section-title">🔬 Core Formulas</div>';  
    ...
    // Code section  
    if (details.code) {  
        html += '<div class="section-title">💻 Core Code</div>';  
        html += `<div class="code-block"><pre>${details.code}</pre></div>`;  
    }  
    
    panel.innerHTML = html;  
    
    // 🔧 Key Fix: Use KaTeX to render each formula directly  
    if (details.formulas && details.formulas.length > 0) {  
...
function setGravityDirection(direction) {  
    ...
    if (direction === 'UP') {  
        ...
        targetGravity = createVector(-GRAVITY_STRENGTH, 0);
    }
...
// ============================================================  
// 💥 Collision Detection & Shatter System  
// ============================================================  
...
// ============================================================  
// Shape Class - Full Implementation  
// ============================================================  
...
function displayUI() {
    ...
    fill(180, 200, 220);
    textSize(10);
    text('🎮 Basic Controls:', 30, 178);
    text('  Click=Shatter | Drag=Launch ✨', 30, 194);
    text('  1/2/3=Add Shape | C=Clear | P=Palette', 30, 210);
    text('  ↑↓←→=Gravity | Space=Zero G', 30, 226);
    
    fill(255, 255, 100);
    textStyle(BOLD);
    textSize(11);
    text('🎬 Q W E R T = Switch Scenes', 30, 246);
    
    fill(100, 255, 200);
    textSize(10);
    text('📚 Press I = Toggle Tech Details', 30, 266);
